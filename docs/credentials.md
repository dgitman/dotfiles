...
Alternatively, store the complete `hosts.yml` file in 1Password and restore it with `make restore`.
...
If application-default credentials need to be restored from 1Password, set `GCLOUD_APPLICATION_DEFAULT_CREDENTIALS_JSON` in `~/.config/op/dotfiles-files.env` and run `make restore`.
