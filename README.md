...
If `~/.local/bin` is on `PATH`, these are also available as `mac-bootstrap`, `brewfile-update`, and `brewfile-cleanup-preview` after initializing submodules and running `~/dotfiles/bin/setup-bin-path.sh`.
